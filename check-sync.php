<?php
/**
 * CARU CARS — Inventory Feed
 *
 * Reads the latest DealerCenter_*.csv on disk and returns the vehicles as
 * a JSON array. Called by the GitHub Actions sync workflow, which wraps
 * the array in inventory-data.js and commits it to the repo.
 *
 * Hostinger is where DC drops the CSV; Netlify serves the JS from GitHub.
 * This endpoint is the bridge between them.
 */
...
if (!$csvs) {
    ...
    die(json_encode(['error' => 'No DealerCenter CSV found', 'hint' => 'DealerCenter FTP push may be broken']));
}
usort($csvs, fn($a, $b) => filemtime($b) <=> filemtime($a));
$csv = $csvs[0];

// 2. Read + force UTF-8. DealerCenter exports CP1252 (Windows-1252).
...
// 4. Sort: newest year first, then priciest first
